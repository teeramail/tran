...
let startDate = moment.tz('2024-05-09 21:00', 'Asia/Bangkok');
const events = [];
...
while (daysGenerated < numberOfDays) {
  ...
  if (startDate.day() === 1 || startDate.day() === 2 || startDate.day() === 4   || startDate.day() === 6 ) {
    events.push({
      ...
      "eventDate": { "$date": { "$numberLong": startDate.valueOf().toString() } },
      "organizerId": "653f2d54a02c73d66e1eec99",
      "tickets": [
        { "type": "VIP unlimitdrink", "price": 1500, "available": 96, "sold": 0 },
        { "type": "Ringside free a drink", "price": 1000, "available": 97, "sold": 0 },
        { "type": "Standard", "price": 600, "available": 494, "sold": 0 }
      ],
      "eventGroup": "cm001",
